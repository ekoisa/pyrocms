...
<?php if ($widgets): ?>
	...
	<table border="0" id="widgets-list" class="table table-striped" cellspacing="0">
		<thead>
		<tr>
			<th width="30"></th>
		...
		<tbody>
			...
			<tr>
				...
				<?php if ($widget->enabled == '1'): ?>
					<?php echo anchor('admin/addons/widgets/disable/' . $widget->id, lang('buttons:disable'), 'class="btn btn-default btn-sm disable"') ?>
				<?php else: ?>
					<?php echo anchor('admin/addons/widgets/enable/' . $widget->id, lang('buttons:enable'), 'class="btn btn-default btn-sm enable"') ?>
				<?php endif ?>
				</td>
			</tr>
			<?php endforeach ?>
