...
<div class="buttons">
	<?php 
		...
		{
			echo anchor('admin/pages/choose_type?modal=true&parent='.$page->id, lang('pages:create_label'), 'class="btn btn-default btn-sm modal"');
		}
		else
		{
			$type_id = $this->db->select('id')->limit(1)->get('page_types')->row()->id;
			echo anchor('admin/pages/create?parent='.$page->id.'&page_type='.$type_id, lang('pages:create_label'), 'class="btn btn-default btn-sm"');
		}

	?>
	<?php echo anchor('admin/pages/duplicate/'.$page->id, lang('pages:duplicate_label'), 'class="btn btn-default btn-sm"') ?>
	<?php echo anchor('admin/pages/edit/'.$page->id, lang('global:edit'), 'class="btn btn-default btn-sm"') ?>
